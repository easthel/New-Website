...
function updateDashboardStats() {
  ...
  document.querySelector('[data-stat="appointments"]').textContent =
    clientData.upcomingAppointments.length;
  document.querySelector('[data-stat="visits"]').textContent =
    clientData.totalVisits;
  document.querySelector('[data-stat="membership"]').textContent =
    clientData.membershipStatus.toUpperCase();
  document.querySelector('[data-stat="points"]').textContent =
    clientData.points.toLocaleString();
}
...
function updateRecentAppointments() {
  ...
  recentList.innerHTML = clientData.recentAppointments
    .map(
      (appointment) => `
        <div class="list-group-item">
            ...
            <small class="text-muted">Service Provider: ${
...
function updateUpcomingServices() {
  ...
  upcomingList.innerHTML = clientData.upcomingAppointments
    .map(
      (appointment) => `
        <div class="list-group-item">
            <div class="d-flex w-100 justify-content-between">
...
function updateProfileInfo() {
  ...
  document.getElementById("membership-expiry").textContent =
    clientData.membershipExpiry;
  document.getElementById("membership-points").textContent =
    clientData.points.toLocaleString();
...
function updateAppointmentsList() {
  const appointmentsList = document.querySelector(
    ".upcoming-appointments-list"
  );
  appointmentsList.innerHTML = clientData.upcomingAppointments
    .map(
      (appointment) => `
        <div class="list-group-item">
            <div class="d-flex w-100 justify-content-between">
...
function updateServiceHistory(filter = "all") {
  ...
  if (filter === "grooming") {
    ...
    filteredHistory = clientData.serviceHistory.filter(
      (service) =>
  ...
  // Get service prices
  const servicePrices = {
    "Full & Basic Grooming": 1500.0,
    "Bath & Blowdry": 800.0,
    Haircut: 1200.0,
    "Nail Trim": 300.0,
    "Ear Cleaning": 400.0,
    "Teeth Brushing": 500.0,
  };
...
function updateNotificationSettings() {
  document.getElementById("emailNotifications").checked =
    clientData.notificationSettings.email;
  document.getElementById("smsNotifications").checked =
    clientData.notificationSettings.sms;
  document.getElementById("appointmentReminders").checked =
    clientData.notificationSettings.appointmentReminders;
  document.getElementById("promotionalOffers").checked =
    clientData.notificationSettings.promotionalOffers;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  scheduleAppointmentButton.addEventListener("click", function () {
    if (appointmentForm.checkValidity()) {
      ...
      const selectedOption = appointmentForm.querySelector(
        `select[name="serviceType"] option[value="${serviceType}"]`
      );
  ...
  document
    .getElementById("filter-grooming")
  ...
  notificationSettingsForm.addEventListener("submit", function (e) {
    ...
    clientData.notificationSettings = {
      ...
      appointmentReminders: document.getElementById("appointmentReminders")
        .checked,
